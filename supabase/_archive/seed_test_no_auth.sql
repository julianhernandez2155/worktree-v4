-- Seed Data for Syracuse University Organizations (TEST VERSION - No Auth Required)
-- This version temporarily disables the foreign key constraint for testing
...
-- Sample Users (Profiles) - No auth.users required with triggers disabled
...
-- First, let's create a temporary table to map skill names to IDs
WITH skill_map AS (
...
SELECT org_id, skill_id, need_type, frequency, gap_level
FROM (
  VALUES
    -- Robotics Club needs
...
DO $$
BEGIN
  RAISE NOTICE 'Test seed data has been loaded successfully!';
  RAISE NOTICE 'NOTE: These test users do NOT have auth accounts - they are for UI testing only';
  RAISE NOTICE 'You can now visit http://localhost:3000/dashboard/org/syracuse-robotics';
  RAISE NOTICE '';
  RAISE NOTICE 'IMPORTANT: This is TEST DATA ONLY! The foreign key constraint was temporarily disabled.';
  RAISE NOTICE 'For production, you must create proper auth.users entries.';
END $$;