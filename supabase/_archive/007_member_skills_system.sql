...
CREATE INDEX idx_task_skills_task ON task_required_skills(task_id);
CREATE INDEX idx_task_skills_skill ON task_required_skills(skill_id);
...
CREATE POLICY "Task creators can manage required skills" ON task_required_skills
    FOR ALL USING (
        EXISTS (
